...
SG=$($AWS_EXECUTABLE ec2 describe-instances --filter "Name=tag:Name,Values=proxy" \
...
if [[ $1 = "pi" ]]
then
    CIDRS=$($AWS_EXECUTABLE ec2 describe-security-groups --group-ids $SG \
...
for ip in $CIDRS; do
	[ "$MYIP/32" != "$ip" ] && $AWS_EXECUTABLE ec2 revoke-security-group-ingress \
		--group-id $SG --protocol tcp --port $PORT --cidr $ip
done

# Allow access for IP of this machine
[ -z $(echo "$CIDRS" | grep "$MYIP/32") ] && $AWS_EXECUTABLE ec2 authorize-security-group-ingress \
    --group-id $SG --protocol tcp --port $PORT --cidr "$MYIP/32"

EC2_PUBLIC_IP=$($AWS_EXECUTABLE ec2 describe-instances --filter "Name=tag:Name,Values=proxy" \
